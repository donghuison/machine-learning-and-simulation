...
contains
   ...
   subroutine bnd__exec_vel(nvar,margin,ix,jx,Vc,g_gamma,x,y,Vci)
      ...
      ! Closed box boundary conditions - zero velocity on all walls
      ...
      if(mpid%f == mnull) then
         call bd_consy(1,margin,0.0d0,Vc(:,:,3),ix,jx)  ! Changed from 1.0d0 to 0.0d0
   ...
   subroutine bnd__exec_prs(margin,ix,jx,pr,g_gamma,x,y)
      ...
      ! Neumann boundary conditions for pressure (closed box)
      if(mpid%l == mnull) then
      ...
      if(mpid%f == mnull) then
         call bd_synpy_car(1,margin,pr,ix,jx)  ! Changed to Neumann BC
      end if
